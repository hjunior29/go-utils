...
// It returns an error if the substring is empty and the string is not empty,
// as this behavior is ambiguous and handled by strings.Count.
//
// Examples:
//
...
// FastDiff returns the elements that are in slice1 but not in slice2.
// It uses generics to work with slices of any comparable type.
// This version is optimized by pre-allocating map and slice capacities for efficiency.
//
// Examples:
//
//	FastDiff([]int{1, 2, 3, 4}, []int{3, 4, 5, 6}) == []int{1, 2}
//	FastDiff([]string{"a", "b", "c"}, []string{"b", "c", "d"}) == []string{"a"}
//	FastDiff([]int{1, 2}, []int{3, 4}) == []int{1, 2}
func FastDiff[T comparable](slice1, slice2 []T) []T {
	// Use a map to store elements of the second slice for efficient lookup.
	// Pre-allocate map capacity for efficiency.
	set2 := make(map[T]struct{}, len(slice2))
	for _, item := range slice2 {
...
// Subtract returns a new slice containing elements that are in slice1 but not in slice2.
// It uses generics to work with slices of any comparable type.
//
// Examples:
//
//	Subtract([]int{1, 2, 3, 4}, []int{3, 4, 5, 6}) == []int{1, 2}
//	Subtract([]string{"a", "b", "c"}, []string{"b", "c", "d"}) == []string{"a"}
//	Subtract([]int{1, 2}, []int{3, 4}) == []int{1, 2}
...
// GroupBy groups elements of a slice into a map based on a key-generating function.
...
//
// @param slice The input slice of elements.
...
// Examples:
//
//	GroupBy([]int{1, 2, 3, 4, 5, 6}, func(n int) string { if n%2 == 0 { return "even" } return "odd" }) == map[string][]int{"odd": {1, 3, 5}, "even": {2, 4, 6}}
//	GroupBy([]string{"apple", "banana", "apricot"}, func(s string) string { return string(s[0]) }) == map[string][]string{"a": {"apple", "apricot"}, "b": {"banana"}}
...
//	SafeValidateIP("invalid-ip") == (false, error)
//	SafeValidateIP("192.168.1.256") == (false, error)
...
// ValidateSocialSecurityNumber checks if a string is a valid U.S. Social Security Number (SSN).
// A valid SSN has the format "###-##-####", where '#' represents a digit.
// It returns an error if the string does not conform to this format or contains invalid characters.
//
// Examples:
//
//	ValidateSocialSecurityNumber("123-45-6789") == nil
//	ValidateSocialSecurityNumber("123456789") returns an error (incorrect format)
//	ValidateSocialSecurityNumber("abc-de-fghi") returns an error (non-digit characters)
//	ValidateSocialSecurityNumber("123-45-678") returns an error (incorrect length)
func ValidateSocialSecurityNumber(ssn string) error {
	if len(ssn) != 11 {
		return errors.New("invalid SSN length: must be 11 characters (###-##-####)")
	}
...
// Examples:
//
//	ValidateDomain("example.com") == nil
//	ValidateDomain("sub.example.co.uk") == nil
//	ValidateDomain("example.io") == nil
//	ValidateDomain("example") returns an error (missing TLD)
//	ValidateDomain("example.c") returns an error (TLD too short)
//	ValidateDomain("example-.com") returns an error (label ends with hyphen)
//	ValidateDomain("-example.com") returns an error (label starts with hyphen)
//	ValidateDomain("example..com") returns an error (empty label)
//	ValidateDomain("example.c_m") returns an error (invalid character in TLD)
//	ValidateDomain("example.123") returns an error (numeric TLD)
//	ValidateDomain("") returns an error (empty string)
...
// FastChunkGeneric splits a slice into smaller slices of a specified size.
// If the last chunk is smaller than the size, it will be returned as is.
// This version is optimized by pre-allocating the result slice and inner slices' capacities.
//
// @param slice The input slice.
...
// Examples:
//
//	FastChunkGeneric([]int{1, 2, 3, 4, 5}, 2) == [][]int{{1, 2}, {3, 4}, {5}}
//	FastChunkGeneric([]string{"a", "b", "c", "d"}, 3) == [][]string{{"a", "b", "c"}, {"d"}}
//	FastChunkGeneric([]int{1, 2, 3}, 1) == [][]int{{1}, {2}, {3}}
//	FastChunkGeneric([]int{1, 2, 3}, 5) == [][]int{{1, 2, 3}}
//	FastChunkGeneric([]int{}, 2) == [][]int{}
//	FastChunkGeneric([]int{1, 2, 3}, 0) returns an error
func FastChunkGeneric[T any](slice []T, size int) ([][]T, error) {
	if size <= 0 {
		return nil, errors.New("chunk size must be greater than 0")
	}
...
// Examples:
//
//	FastGroupBy([]int{1, 2, 3, 4, 5, 6}, func(n int) string { if n%2 == 0 { return "even" } return "odd" }) == map[string][]int{"odd": {1, 3, 5}, "even": {2, 4, 6}}
//	FastGroupBy([]string{"apple", "banana", "apricot"}, func(s string) string { return string(s[0]) }) == map[string][]string{"a": {"apple", "apricot"}, "b": {"banana"}}
func FastGroupBy[T any, K comparable](slice []T, keyFunc func(T) K) map[K][]T {
	// Pre-allocate map capacity for efficiency.
	grouped := make(map[K][]T, len(slice)) // Heuristic: capacity based on slice length
	...
	for _, item := range slice {
		...
		if _, ok := grouped[key]; !ok {
			grouped[key] = make([]T, 0, len(slice)/2) // Heuristic capacity
		}
		grouped[key] = append(grouped[key], item)
	}
	return grouped
...
func FastMap[T any, U any](slice []T, f func(T) U) []U {
	result := make([]U, len(slice))
	// Pre-allocate capacity for the result slice can improve performance
...
// @return An error if the string is empty or contains non-alphanumeric characters, otherwise nil.
//
// Examples:
//
//	ValidateAlphaNumeric("HelloWorld123") == nil
//	ValidateAlphaNumeric("Hello World") returns an error (contains a space)
//	ValidateAlphaNumeric("") returns an error
func ValidateAlphaNumeric(s string) error {
	if s == "" {
		return errors.New("string cannot be empty")
	}
...
// @return The string found between the start and end delimiters, or an empty string if not found.
//
// Examples:
//
//	Between("hello [world]!", "[", "]") == "world"
...
// It currently checks for a minimum length and the presence of at least one uppercase letter,
...
func ValidatePasswordStrength(password string) error {
	...
	var (
		...
		hasLower    bool
		hasDigit    bool
		hasSpecial  bool // Example of extending criteria: checking for special characters
	)
	...
	// Example extension: check for special characters
	// if !hasSpecial {
...
// DiffGeneric returns a new slice containing elements that are in slice1 but not in slice2.
// It uses generics to work with slices of any comparable type.
// The order of elements in the resulting slice is preserved from slice1.
//
// Examples:
...
//	SplitOnceGeneric([]string{"a", "b", "c"}, []string{"x"}) == [][]string{{"a", "b", "c"}, {}}
//	SplitOnceGeneric([]int{1, 2}, []int{}) == [][]int{{}, {1, 2}}
func SplitOnceGeneric[T comparable](slice []T, sep []T) ([][]T, error) {
	if len(sep) == 0 {
		if len(slice) != 0 {
			// The behavior of splitting by an empty separator is complex.
			// For consistency with strings.Split, we return an error if the string is not empty.
...
// @return An error if the email format is invalid, otherwise nil.
//
// Examples:
//
//	ValidateEmail("test@example.com") == nil
...
// SafeKeys returns a slice of all keys in a map, along with a nil error.
// The order of keys in the returned slice is not guaranteed.
//
// @param m The input map.
// @return A slice containing all keys from the input map, and a nil error.
//
// Examples:
//
//	SafeKeys(map[string]int{"a": 1, "b": 2}) == ([]string{"a", "b"}, nil) (order may vary)
//	SafeKeys(map[int]string{1: "one", 2: "two"}) == ([]int{1, 2}, nil) (order may vary)
//	SafeKeys(map[string]int{}) == ([]string{}, nil)
func SafeKeys[K comparable, V any](m map[K]V) ([]K, error) {
	keys := make([]K, 0, len(m))
	for k := range m {
		keys = append(keys, k)
	}
...
// SafeValues returns a slice of all values in a map, along with a nil error.
// The order of values in the returned slice is not guaranteed and corresponds
// to the order of keys returned by iterating over the map.
//
// @param m The input map.
// @return A slice containing all values from the input map, and a nil error.
//
// Examples:
//
//	SafeValues(map[string]int{"a": 1, "b": 2}) == ([]int{1, 2}, nil) (order may vary)
//	SafeValues(map[int]string{1: "one", 2: "two"}) == ([]string{"one", "two"}, nil) (order may vary)
//	SafeValues(map[string]int{}) == ([]int{}, nil)
func SafeValues[K comparable, V any](m map[K]V) ([]V, error) {
	values := make([]V, 0, len(m))
	for _, v := range m {
		values = append(values, v)
	}
...
//	SafeFind([]int{}, func(n int) bool { return n > 0 }) == (0, false)
func SafeFind[T any](slice []T, predicate func(T) bool) (T, bool) {
	for _, item := range slice {
		if predicate(item) {
			return item, true
...
// SafeFindIndex returns the index of the first element in a slice that satisfies a given predicate function.
// It returns the index and a nil error if the element is found.
// It returns -1 and a nil error if no element satisfies the predicate.
//
// @param slice The input slice of elements of type T.
// @param predicate A function that takes an element of type T and returns a boolean.
// @return The index of the first element that satisfies the predicate and a nil error, or -1 and a nil error if no such element is found.
//
// Examples:
//
//	SafeFindIndex([]int{1, 2, 3, 4, 5}, func(n int) bool { return n%2 == 0 }) == (1, nil)
//	SafeFindIndex([]string{"a", "b", "c"}, func(s string) bool { return s == "d" }) == (-1, nil)
//	SafeFindIndex([]int{}, func(n int) bool { return n > 0 }) == (-1, nil)
func SafeFindIndex[T any](slice []T, predicate func(T) bool) (int, error) {
	for i, item := range slice {
		if predicate(item) {
			return i, nil
		}
...
// SafeFindLastIndex returns the index of the last element in a slice that satisfies a given predicate function.
// It returns the index and a nil error if the element is found.
// It returns -1 and a nil error if no element satisfies the predicate.
//
// @param slice The input slice of elements of type T.
// @param predicate A function that takes an element of type T and returns a boolean.
// @return The index of the last element that satisfies the predicate and a nil error, or -1 and a nil error if no such element is found.
//
// Examples:
//
//	SafeFindLastIndex([]int{1, 2, 3, 4, 5}, func(n int) bool { return n%2 == 0 }) == (3, nil)
//	SafeFindLastIndex([]string{"a", "b", "c"}, func(s string) bool { return s == "d" }) == (-1, nil)
//	SafeFindLastIndex([]int{}, func(n int) bool { return n > 0 }) == (-1, nil)
func SafeFindLastIndex[T any](slice []T, predicate func(T) bool) (int, error) {
	for i := len(slice) - 1; i >= 0; i-- {
		if predicate(slice[i]) {
			return i, nil
		}
...
// is ambiguous and handled by strings.Split.
//
// @param slice The slice to split. The elements must be of a comparable type.
// @param sep The separator slice. The elements must be of the same type as the slice elements.
// @return A slice containing two slices: the part before the separator and the part after, or an error if the operation fails.
//
// Examples:
//
//	SafeSplitOnceGeneric([]int{1, 2, 3, 4, 5}, []int{3, 4}) == ([][]int{{1, 2}, {5}}, nil)
...
func SafeCountLines(s string) (int, error) {
	...
	// For consistency, we'll mimic that behavior by checking if the string ends with a newline.
	if strings.HasSuffix(s, "\n") && count > 1 {
		// If it ends with a newline and we counted more than one
...
//
// @param s The input string to check.
// @param defaultVal The default string value to return if `s` is empty or whitespace.
// @return The input string `s` if it's not empty or whitespace, otherwise `defaultVal`.
//
// Examples:
//
//	DefaultIfEmpty("hello", "default") == "hello"
